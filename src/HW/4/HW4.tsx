...
export const HW4 = () => {
  ...
  return (
    <div id={'hw04'}>
      ...
      <Input currentText = {currentText} setCurrentText = {setCurrentText}/>  {/*НЕ ХВАТАЕТ АРГУМЕНТОВ */}

      <Button name = {"добавить"} callBack = {handleSave}  />  {/*НЕ ХВАТАЕТ АРГУМЕНТОВ */}
      ...
      <ol id={'hw04-tasks'}>
       {texts.map((el, index) => {
         return (
           <li key={index} id={`hw04-task-${index}`} className={index % 2 ? '' : S.chetNechet}>{/*чет нечет */}
             {el}
           </li>
         );
       })}
